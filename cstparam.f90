...
!       CALL SETSAIRFOILCOORDINATES(IU,NX,NY,NPROF,NDEG,NL,AL,ZETAL,NU,AU,ZETAU,XINTERIOR)
...
PROGRAM TESTECST
  ...
  END IF
  IU=IUNIT()  
  OPEN(iu,file="airfoil.poly",status="unknown")
  NX=40
  NY=40
  NPROF=300
  SELECT CASE(mode)
  CASE(1)
    WRITE(*,*)" 111 "
    CALL READUIUCPROF("profile.dat",NL,XL,ZL,NU,XU,ZU,xinterior)
    KUP=IUNIT()
    OPEN(KUP,FILE="parameters.txt",status="unknown")
    CALL DETERMINESA(NL,NDEG,XL,ZL,AL,ZETAL)
    WRITE(kup,"(100E16.5)",advance="no")al(1:ndeg+1),zetal
    CALL DETERMINESA(NU,NDEG,XU,ZU,AU,ZETAU)
    WRITE(kup,"(100E16.5)",advance="no")au(1:ndeg+1),zetau
    CLOSE(kup)
  CASE(2)
    WRITE(*,*)" 222 "
    kup=iunit()
    ...
    WRITE(*,*)"al,zetal,au,zetau",al,zetal,au,zetau
    CLOSE(kup)
  END SELECT
  CALL SETSAIRFOILCOORDINATES(IU,NX,NY,NPROF,NDEG,AL,ZETAL,AU,ZETAU,XINTERIOR)
END PROGRAM TESTECST
